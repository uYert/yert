...
class Connect4:
	def __init__(self, ctx,  players, columns=7, rows=6):
		...
		self.react_emojis = {str(n) + '\N{variation selector-16}\N{combining enclosing keycap}': n-1 for n in range(1, columns+1)}
		self.plays = 0
	...
	def check_winner(self) -> bool:
		...
		for array_index, array in enumerate(self.grid):
			for index, item in enumerate(array):
				if (array_index < self.rows and
						self.grid[index][array_index] == self.current_player.mark):
					row_counter += 1
				else:
					row_couter = 0
				if (index < self.columns and
						self.grid[array_index][index] == self.current_player.mark):
					column_counter += 1
				else:
					column_counter = 0
	...
	async def play(self):
		self.message = await self.ctx.send(f"{self.current_player.player.mention}, it's your turn !\n```{self.draw_grid()}```")
		...
		while True:
			...
			if self.plays >= 8 and self.check_winner():
				await self.ctx.send(f"Congratulations ! {self.current_player.player.mention} has won the game.")
			else:
				self.current_player = next(self.players)
				await self.message.edit(content=f"{self.current_player.player.mention}, it's your turn !\n```{self.draw_grid()}```")
	...
	async def get_input(self) -> str:
		def check(reaction, user):
			return user.id == self.current_player.player.id and str(reaction.emoji) in self.react_emojis
		reaction, user = await self.ctx.bot.wait_for("reaction_add", check=check, timeout=30.0)
